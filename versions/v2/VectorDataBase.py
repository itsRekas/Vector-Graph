...
class VectorDataBase:
    ...
    def __init__(self, database_name : str, host : str, port : int,  embedding_model : str, target_embedding_dim: int):
        ...
        # For v2: we embed S, P, O separately and concatenate, so dimension is 3x model dim
        ...
        # V2: Embedding is concatenation of subject, predicate, object embeddings (3x model dim)
    ...
    def _extract_triple_from_sparql(sparql_query: str) -> Optional[str]:
        ...
        Handles variables (?var) and URIs/literals.
        Returns the triple in format: <subject> <predicate> <object> (no period)
        For variables in subject, returns "?VAR <predicate> <object>" format.
        Returns None if no valid triple pattern found.
        """
        # Pattern to match triple in SPARQL with variables:
        # ?var <p> <o> or <s> <p> <o> or <s> <p> "literal" or ?var <p> "literal"
        # This regex matches: (?var|<uri>) <uri> (<uri>|"literal")
        # Group 1: variable name (if variable)
        # Group 2: subject URI (if URI)
        # Group 3: predicate URI
        # Group 4: object URI (if URI)
        # Group 5: object literal (if literal)
        pattern = r'(?:\?(\w+)|<([^>]+)>)\s+<([^>]+)>\s+(?:<([^>]+)>|"([^"]+)")'
        ...
        if match:
            ...
            if match.group(4):  # URI
                obj = f"<{match.group(4)}>"
            else:  # Literal
                obj = f'"{match.group(5)}"'
            
            # Return in format: <s> <p> <o> or ?VAR <p> <o>
            return f"{subject} {predicate} {obj}"
    ...
    def _encode_text_batch(self, texts: Sequence[str], normalize: bool = False) -> np.ndarray:
        ...
        # V2: Try to disable normalization if the model supports it
        try:
            embeddings = self._embedding_model.encode(
                ...
                normalize_embeddings=False  # V2: Don't normalize embeddings
            )
        except (TypeError, ValueError) as e:
            # Fallback: model doesn't support normalize_embeddings parameter
            # Some models normalize by default, we'll use them as-is
            embeddings = self._embedding_model.encode(
                texts, 
                convert_to_numpy=True,
                show_progress_bar=False
        ...
        # V2: Check if embeddings are normalized and denormalize if needed
        # This ensures consistency with stored embeddings (which should not be normalized)
        norms = np.linalg.norm(embeddings, axis=1)
        if np.allclose(norms, 1.0, atol=0.01):
            # Embeddings are normalized (norm ~1.0), scale them to match non-normalized format
            # Use a scaling factor that matches typical non-normalized embedding magnitudes
            # Based on test output, non-normalized embeddings have norm ~3.7
            scale_factor = 3.7  # Approximate norm of non-normalized embeddings
            embeddings = embeddings * scale_factor
        
        # V2: No normalization - embeddings stored as-is (now denormalized if needed)
        
        return embeddings
    
    def _embed_triple_parts(self, subject: str, predicate: str, obj: str) -> np.ndarray:
        """
        Embed subject, predicate, and object separately, then concatenate.
        V2 approach: [subject_emb | predicate_emb | object_emb]
        ...
        Returns:
            Concatenated embedding vector of shape (3 * model_dim,)
        """
        # Embed each part separately
    ...
    def _embed_query_pattern(self, subject: Optional[str], predicate: Optional[str], obj: Optional[str]) -> np.ndarray:
        """
        Embed a query pattern where missing parts (variables) are replaced with zero vectors.
        V2 approach: [subject_emb | predicate_emb | object_emb] with zeros for variables.
        ...
        Returns:
            Concatenated embedding vector of shape (3 * model_dim,)
        """
        # Create zero vector for missing parts (shape: model_dim,)
        ...
        # Embed non-None parts (returns 2D array: (n_parts, model_dim))
        if parts_to_embed:
        ...
        # Each part is either a zero_vector or the corresponding embedding
        result_parts = []
        embed_idx = 0  # Index into embeddings array
        
        for part in parts_list:
            if part is None:
                # Missing part: use zero vector
                result_parts.append(zero_vector)
            else:
                # Present part: use its embedding
                if embed_idx < len(embeddings):
                    # Get the embedding for this part (shape: model_dim,)
    ...
    def insert_data_from_file(self, filename : str, collection_name : str, log : bool = False, chunk_size : int  = 100 ) -> Union[MutationResult, List]:
        """
        Insert triples from .nt file using V2 approach: parse into S, P, O, embed each separately, concatenate.
        ...
        try:
            ...
            for start in range(0, len(data), chunk_size):
                ...
                # V2: Parse each triple into S, P, O, embed separately, concatenate
                embeddings_list = []
                valid_triples = []
                ...
                for triple_text in chunk:
                    ...
                    if parsed:
                        ...
                        # Embed S, P, O separately and concatenate
        ...
        try:
            ...
            for query in query_list:
                ...
                if isinstance(query, str) and ('SELECT' in query.upper() or 'WHERE' in query.upper()):
                    ...
                    if triple:
                        # Check if triple has variable in subject (starts with ?)
                        if triple.startswith("?"):
                            # Format: "?VAR <predicate> <object>"
                            parts = triple.split(None, 2)  # Split on whitespace, max 2 splits
                            if len(parts) == 3:
                                # Subject is variable (None), predicate and object are present
                                if log:
                                    print(f"DEBUG: SPARQL triple split - parts={parts}, creating pattern: (None, {parts[1]}, {parts[2]})")
                                query_patterns.append((None, parts[1], parts[2]))
                            else:
                            ...
                            # Normal triple: parse it
                            parsed = self._parse_triple(triple)
                            if parsed:
                                query_patterns.append(parsed)
                            else:
                                # If parsing fails, treat as full triple string
                                query_patterns.append((triple, None, None))
                    else:
