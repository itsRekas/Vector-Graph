...
VERSION MAPPING:
- VectorDatabase Implementation: lib.VectorDataBase_v_1.VectorDataBase
- Collection Name: "lubm_graph_v_1"
- Embedding Strategy: Raw triple text embedding (no S|P|O separation)
- Embedding Dimension: model_dim (384 for all-MiniLM-L6-v2)
...
from lib.VectorDataBase_v_1 import VectorDataBase
import re
...
# V1: Uses VectorDataBase_v_1 which embeds triples as raw text (no S|P|O separation)
# Using faster model: all-MiniLM-L6-v2 (faster than L12, slightly less accurate)
# Note: all-MiniLM-L6-v2 outputs 384 dimensions (same as L12), so we keep 384
    ...
    try:
        ...
        if vdb is not None and len(search_queries) > 0:
            try:
                ...
                # Extract query strings from search_queries
                query_strings = [q.get('text', '') for q in search_queries if q.get('text')]
                ...
                vector_results = vdb.search(
                    collection_name="lubm_graph_v_1",  # Update collection name
                    query_texts=query_strings,  # Pass raw query strings
                    limit=k_int,
                    output_fields=["text"],
                    log=True
                )
                    ...
                    if value_row_idx < len(vector_results):
                        ...
                        for match_idx, match in enumerate(matches):
                            ...
                            if triple_data:
                                ...
                                # Use variable names WITHOUT ? prefix for SPARQL Results JSON format
                                row = {}
                                for var in variables:
                                    var_name = var.lstrip('?')  # Remove ? prefix for key
                                    role = pattern_variable_roles.get(var_name)
                                    
                                    # First, try to get from current pattern
                                    if role == 'subject':
                                        row[var_name] = {"type": "uri", "value": triple_data['subject']}
                                    elif role == 'predicate':
                                        row[var_name] = {"type": "uri", "value": triple_data['predicate']}
                                    elif role == 'object':
                                        obj_type = 'literal' if triple_data['object_type'] == 'literal' else 'uri'
                                        row[var_name] = {"type": obj_type, "value": triple_data['object']}
                                    ...
                                    # Try both with and without "?" prefix, and case variations
                                        ...
                                        if term_json is not None:
                                            if isinstance(term_json, dict):
                                                ...
                                                row[var_name] = {"type": "uri", "value": term_json}
        ...
        # Return format expected by comunica-vector: {"vars": [...], "rows": [...]}
        # comunica-vector expects json.rows (vars is optional but included for consistency with V2)
        clean_vars = [v.lstrip('?') for v in variables]
        response = {
            "vars": clean_vars,
        ...
        print(f"Response structure: rows count={len(all_results_rows)}")
        if len(all_results_rows) > 0:
            print(f"First row keys: {list(all_results_rows[0].keys())}")
            print(f"First row sample: {json.dumps(all_results_rows[0], indent=2)[:200]}")
