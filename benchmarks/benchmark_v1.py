...
def run_comunica_vector(query: str, k: int) -> Tuple[int, float]:
    ...
    try:
        ...
        try:
            ...
            if k == 10:  # Only print debug for first k value to avoid spam
                ...
                if isinstance(output, dict):
                    ...
                    if 'results' in output and 'bindings' in output['results']:
                        print(f"    DEBUG: bindings type: {type(output['results']['bindings'])}, length: {len(output['results']['bindings']) if isinstance(output['results']['bindings'], list) else 'N/A'}")
                    if 'rows' in output:
            ...
            if isinstance(output, dict):
                if 'results' in output and 'bindings' in output['results']:
                    if isinstance(output['results']['bindings'], list):
                        count = len(output['results']['bindings'])
                    else:
                        print(f"    WARNING: 'bindings' is not a list, type: {type(output['results']['bindings'])}")
                        count = 0
                ...
                # Check for 'vars' and 'rows' structure (from vector endpoint)
                    ...
                    print(f"    WARNING: Unknown response format, keys: {list(output.keys())}")
                    count = 0
            elif isinstance(output, list):
                # Response is directly a list (empty array or list of results)
                count = len(output)
            else:
...
def create_visualization(results: Dict[str, Dict], queries_info: List[Dict[str, str]] = None, output_file: str = "benchmark_results_v_1.png"):
    """
    ...
    def create_single_plot(query_list, title_suffix, filename_suffix):
        ...
        # For complex queries, only show k=1000; for simple queries, show all k values
        is_complex = filename_suffix == "complex"
        k_values_to_show = [1000] if is_complex else K_VALUES
        
        fig, ax = plt.subplots(figsize=(max(12, len(query_list) * 2), 6))
        
        # Prepare data
        x = np.arange(len(query_list))
        n_bars = len(k_values_to_show) + 1  # k values + comunica-sparql-file
        width = 0.8 / n_bars  # Width of bars to fit all bars in group
        
        # Create bars for each k value (only those that were tested)
        bars = []
        colors = plt.cm.tab10(np.linspace(0, 1, len(K_VALUES)))
        k_color_map = {k: colors[i] for i, k in enumerate(K_VALUES)}
        ...
        for i, k in enumerate(k_values_to_show):
            ...
            bar = ax.bar(pos, values, width, label=f'k={k}', alpha=0.8, color=k_color_map[k])
            bars.append(bar)
        
        # Add comunica-sparql-file bar (last position)
        file_offset = (len(k_values_to_show) - (n_bars - 1) / 2) * width
        ...
        ax.set_title(f'Query Results Comparison: Vector (k values) vs comunica-sparql-file - {title_suffix} (V1)', fontsize=14, fontweight='bold')
        ax.set_xticks(x)
...
def main():
    ...
    print("=" * 70)
    print("Comunica Benchmark Script - V1")
    ...
    for query_info in queries:
        ...
        is_simple = is_simple_query(query_name, query_text)
        
        # For complex queries, only test k=1000; for simple queries, test all k values
        k_values_to_test = [1000] if not is_simple else K_VALUES
        
        # Initialize all k values to 0 (for visualization consistency)
        for k in K_VALUES:
    ...
    for query_name in sorted(results.keys()):
        print(f"\n{query_name}:")
        # Check if this is a complex query (only k=1000 was tested)
        query_info = next((q for q in queries if q['name'] == query_name), None)
        is_simple = query_info and is_simple_query(query_name, query_info['query'])
        
        for k in K_VALUES:
            count = results[query_name][f'k_{k}']['count']
            time = results[query_name][f'k_{k}']['time']
            if not is_simple and k != 1000:
                print(f"  k={k:3d}: (skipped for complex query)")
            else:
    ...
    output_json = "benchmark_results_v_1.json"
