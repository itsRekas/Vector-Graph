...
VERSION MAPPING:
- VectorDatabase Implementation: lib.VectorDataBase_v_3.VectorDataBase
- Collection Name: "lubm_graph_v_3"
...
from lib.VectorDataBase_v_3 import VectorDataBase
import re
...
# V3: Uses VectorDataBase_v_3 which embeds S, P, O separately, concatenates, and normalizes
    ...
    try:
        ...
        if vdb is not None and len(search_queries) > 0:
            try:
                ...
                # V3: Pass query dicts with subject/predicate/object for proper S|P|O embedding
                ...
                print(f"V3: Passing {len(query_dicts)} query patterns with S|P|O structure")
                
                vector_results = vdb.search(
                    collection_name="lubm_graph_v_3",  # V3 collection
                    query_texts=query_dicts,  # V3: Pass dicts with subject/predicate/object
                    limit=k_int,
                    output_fields=["text"],
                    log=True
                )
    ...
    if query:
        ...
        if query_upper.startswith('SELECT'):
            ...
            if vdb is not None:
                try:
                    collection_name = "lubm_graph_v_3"  # V3 collection
                    # V3: Try to extract triple pattern from SPARQL query for proper S|P|O embedding
                    ...
                    if triple:
                        ...
                        if len(parts) == 3:
                            ...
                            # Pass as dict for V3 embedding (handles variables in all positions)
                            query_dict = {
                                'subject': subject,
                                'predicate': predicate,
                                'object': obj
